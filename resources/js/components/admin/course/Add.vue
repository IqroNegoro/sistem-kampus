            ...
            <form @submit.prevent="form.post(route('courses.store'), {
                ...
                <label class="block text-sm">
                    ...
                    <input v-model="form.code" type="text"
                        class="block w-full mt-1 text-sm border dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input" :class="{'border-red-500 dark:border-red-500': form.errors.code}"
                        placeholder="W001" />
                        <p class="text-red-500 font-medium" v-if="form.errors.code"> {{ form.errors.code }} </p>
                </label>
                <label class="block text-sm">
                    <span class="text-gray-700 dark:text-gray-400">SKS</span>
                    <input v-model="form.sks" type="number"
                        class="block w-full mt-1 text-sm border dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input" :class="{'border-red-500 dark:border-red-500': form.errors.sks}"
                        placeholder="3" />
                        <p class="text-red-500 font-medium" v-if="form.errors.sks"> {{ form.errors.sks }} </p>
                </label>
                <label class="block text-sm">
...
onMounted(async () => {
    studies.value = await axios.get(route("studies.get")).then(res => res.data);
});
</script>